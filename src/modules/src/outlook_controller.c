...
{
  pidSetDesired(&pidRoll, rollRateDesired);
  rollOutput = saturateSignedInt16(pidUpdate(&pidRollRate, rollRateActual, true));

  pidSetDesired(&pidPitch, pitchRateDesired);
...
//**Change the function below ? Changes start here? ---Lin
**//
...
{
    pidReset(&pidPitch);//something wrong here, from pidroll->pidpitch
}
